merge_all_tidy <- function(males,
  ...
  DT <- data.table::rbindlist(l, idcol = FALSE)
  ...
  if (!is.null(sum_variable)) {

    sum_variable <- eval(sum_variable)


    out <- DT[!is.na(sum_variable),
              (new_var_name = sum(value)),
              by = c(sum_variable, "Sex")]
